...
export class DeleteTitleDialog extends DialogController {
                ...
                UIDeleteContext((deleteFunc, data, isLoading) =>
                    VStack({ alignment: cTopLeading, spacing: 10 })(
                        HStack({ alignment: cTopLeading })(
                            Text('Delete Title').fontSize(24).fontWeight('500'),
                            Spacer(),
                            Icon('\\e5cd').size(20).cursor('pointer').onClick(() => this.OnCancel())
                        ).height(50),

                        Text(`Are you sure you want to delete **${(data as any)?.title_name}** ?`).render(RenderingTypes.Markdown),
